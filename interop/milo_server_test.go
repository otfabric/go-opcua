...
// TestMiloServer_WriteEncodingMask verifies Milo rejects Status/timestamp Writes.
// Soft-accept of timestamp Writes is a promotion blocker (do not skip).
func TestMiloServer_WriteEncodingMask(t *testing.T) {
	h := startMiloServer(t)
	c := dialClient(t, h.endpoint)
	ctx, nsIdx := findNS(t, c)
	nodeID := ua.NewStringNodeID(nsIdx, "Access.ReadWrite")
	...
	if bad.Results[0] != ua.StatusBadWriteNotSupported {
		// Capability claim is strict: EncodingMask Writes must be rejected with
		// BadWriteNotSupported. Milo soft-accepts → unsupported for this claim
		// (documented peer variance), not an open peer-capability-unknown.
		t.Logf("timestamp write: Milo returned %v (soft-accept); write.encoding-mask C→M = unsupported for BadWriteNotSupported claim",
			bad.Results[0])
		return
	}
	t.Logf("Milo rejects timestamp Writes with BadWriteNotSupported — eligible to promote verified")
}
