...
func TestCoverageManifestValid(t *testing.T) {
	...
	allowedUnverifiedReasons := map[string]bool{
		"test-missing":            true,
	...
	for _, e := range cov.Entries {
		...
		case "unverified":
			if e.Reason == "" {
				t.Errorf("%s/%s: unverified missing controlled reason", e.Capability, e.Direction)
			} else if !allowedUnverifiedReasons[e.Reason] {
				t.Errorf("%s/%s: invalid unverified reason %q", e.Capability, e.Direction, e.Reason)
			}
		}
	}
