...
func (e *ExpandedNodeID) Encode() ([]byte, error) {
	buf := NewBuffer(nil)
	if e == nil {
		return []byte{}, fmt.Errorf("%w: ExpandedNodeID is nil", errors.ErrInvalidNodeID)
	}
	if e.NodeID == nil {
		buf.WriteInt16(0)
	} else {
...
func ParseExpandedNodeID(s string, ns []string) (*ExpandedNodeID, error) {
	...
	case strings.HasPrefix(nsval, "nsu="):
		if ns == nil {
			return nil, fmt.Errorf("%w: namespace urls require a server NamespaceArray", errors.ErrInvalidNamespace)
		}
		...
		if !ok {
			return nil, fmt.Errorf("%w: namespace uri %s not found in %#v", errors.ErrNamespaceNotFound, nsval, ns)
		}

	case strings.HasPrefix(nsval, "ns="):
		n, err := strconv.Atoi(nsval[3:])
		if err != nil {
			return nil, fmt.Errorf("%w: %s", errors.ErrInvalidNamespace, s)
		}
		if n < 0 || n > math.MaxUint16 {
			return nil, fmt.Errorf("%w: out of range (0..65535): %s", errors.ErrInvalidNamespace, s)
		}
		nsid = uint16(n)

	default:
		return nil, fmt.Errorf("%w: %s", errors.ErrInvalidNodeID, s)
	}
	...
	case strings.HasPrefix(idval, "i="):
		...
		if err != nil {
			return nil, fmt.Errorf("%w: invalid numeric id: %s", errors.ErrInvalidNodeID, s)
		}
		switch {
		case nsid == 0 && id < 256:
		...
		default:
			return nil, fmt.Errorf("%w: numeric id out of range (0..2^32-1): %s", errors.ErrInvalidNodeID, s)
		}
	...
	case strings.HasPrefix(idval, "g="):
		...
		if n == nil || n.StringID() == "" {
			return nil, fmt.Errorf("%w: invalid guid: %s", errors.ErrInvalidNodeID, s)
		}
	...
	case strings.HasPrefix(idval, "b="):
		...
		if err != nil {
			return nil, fmt.Errorf("%w: invalid opaque id: %s", errors.ErrInvalidNodeID, s)
		}
