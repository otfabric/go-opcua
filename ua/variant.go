...
func NewVariant(v interface{}) (*Variant, error) {
	va := &Variant{}
	if !isBuiltinType(v) {
		return nil, fmt.Errorf("%w: %s", errors.ErrUnsupportedType, reflect.ValueOf(v).Type().Name())
	}
...
// VariantAs returns the variant value as the given type.
// It returns an error if the value cannot be converted to the requested type.
func VariantAs[T any](v *Variant) (T, error) {
	val, ok := v.value.(T)
	if !ok {
		var zero T
		return zero, fmt.Errorf("%w: got %T, want %T", errors.ErrUnsupportedType, v.value, zero)
	}
	return val, nil
}
...
func (m *Variant) Decode(b []byte) (int, error) {
	...
	if !ok {
		return buf.Pos(), fmt.Errorf("%w: id=%d", errors.ErrUnsupportedType, m.Type())
	}
...
func (m *Variant) decodeValue(buf *Buffer) interface{} {
	...
	case TypeIDDataValue:
		...
		return v
	case TypeIDVariant:
		// NOTE: nested Variants could cause deep recursion from malicious input.
		// A depth limit requires threading a counter through the decode interface;
		// tracked as a known limitation.
		v := new(Variant)
		buf.ReadStruct(v)
		return v
	case TypeIDDiagnosticInfo:
		// NOTE: nested DiagnosticInfo could cause deep recursion from malicious input.
		// Same limitation as Variant above.
...
var errUnbalancedSlice = errors.ErrUnbalancedArray
...
// String returns a human-readable representation of the variant value.
// For arrays, elements are comma-separated and enclosed in brackets.
func (m *Variant) String() string {
	return m.formatValue()
}

// formatValue formats a single or array variant value.
func (m *Variant) formatValue() string {
	if m.value == nil || m.Type() == TypeIDNull {
		return "null"
	}

	if m.Has(VariantArrayValues) {
		return m.formatArray()
	}

	return m.formatScalar(m.value)
}

// formatScalar formats a single scalar value.
func (m *Variant) formatScalar(v interface{}) string {
	switch x := v.(type) {
	case bool:
		if x {
			return "true"
	...
	case *ExpandedNodeID:
		...
		return x.NodeID.String()
	case StatusCode:
		if d, ok := StatusCodes[x]; ok {
			return d.Name
		}
	...
	case *QualifiedName:
		...
		return fmt.Sprintf("%d:%s", x.NamespaceIndex, x.Name)
	case *LocalizedText:
		if x == nil {
			return "null"
		}
	...
	case *ExtensionObject:
		...
		return fmt.Sprintf("ExtensionObject(TypeID=%s)", x.TypeID.NodeID)
	case *DataValue:
		if x == nil {
			return "null"
		}
...
// formatArray formats an array variant as "[elem1, elem2, ...]".
func (m *Variant) formatArray() string {
	...
	var parts []string
	for i := 0; i < val.Len(); i++ {
		parts = append(parts, m.formatScalar(val.Index(i).Interface()))
	}
