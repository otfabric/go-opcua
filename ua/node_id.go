...
type NodeID struct {
	// mask stores the encoding type (bits 0-3) and optional flags:
	// bit 7 = NamespaceURI present, bit 6 = ServerIndex present.
	mask NodeIDType
	ns   uint16
	nid  uint32
	bid  []byte
...
func ParseNodeID(s string) (*NodeID, error) {
	...
	if id.HasNamespaceURI() {
		return nil, fmt.Errorf("%w: namespace uris not supported, use ParseExpandedNodeID", errors.ErrInvalidNodeID)
	}
	if id.HasServerIndex() {
		return nil, fmt.Errorf("%w: server index not supported, use ParseExpandedNodeID", errors.ErrInvalidNodeID)
	}
...
func (n *NodeID) SetNamespace(v uint16) error {
	...
	case NodeIDTypeTwoByte:
		if v != 0 {
			return fmt.Errorf("%w: out of range [0..0]: %d", errors.ErrInvalidNodeID, v)
		}
		return nil

	case NodeIDTypeFourByte:
		if max := uint16(math.MaxUint8); v > max {
			return fmt.Errorf("%w: out of range [0..%d]: %d", errors.ErrInvalidNodeID, max, v)
		}
		n.ns = uint16(v)
		return nil
...
func (n *NodeID) SetIntID(v uint32) error {
	...
	default:
		return fmt.Errorf("%w: incompatible node id type", errors.ErrInvalidNodeID)
	}
}
...
func (n *NodeID) Decode(b []byte) (int, error) {
	...
	default:
		return 0, fmt.Errorf("%w: type=%v", errors.ErrInvalidNodeID, typ)
	}
}
...
func (n *NodeID) Encode() ([]byte, error) {
	...
	default:
		return nil, fmt.Errorf("%w: type=%v", errors.ErrInvalidNodeID, n.Type())
	}
	return buf.Bytes(), buf.Error()
}
...
// UnmarshalXML decodes a NodeID from the canonical OPC-UA XML format (Part 6 §5.2.2.10).
