...
func DefaultDialer() *uacp.Dialer {
	// Clone DefaultClientACK so options like ReceiveBufferSize cannot mutate
	// the package-level defaults shared by other clients/tests in-process.
	ack := *uacp.DefaultClientACK
	return &uacp.Dialer{
		Dialer: &net.Dialer{
