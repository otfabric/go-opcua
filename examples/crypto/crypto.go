...
var (
	...
	appuri   = flag.String("app-uri", "urn:otfabric:opcua:client", "Application URI")
...
func clientOptsFromFlags(endpoints []*ua.EndpointDescription) []opcua.Option {
	...
	if *gencert || (*certfile != "" && *keyfile != "") {
		...
		slog.Debug("loading cert/key", "cert", *certfile, "key", *keyfile)
...
func authFromFlags(cert []byte, pk *rsa.PrivateKey) (ua.UserTokenType, []opcua.Option) {
	...
	case "issuedtoken":
		log.Printf("warning: issued token authentication is not fully implemented")
...
func validateEndpointConfig(endpoints []*ua.EndpointDescription, secPolicy string, secMode ua.MessageSecurityMode, authMode ua.UserTokenType) error {
	...
	err := fmt.Errorf("%w: server does not support an endpoint with security : %s , %s, %s", errors.ErrNoMatchingEndpoint, secPolicy, secMode, authMode)
	printEndpointOptions(endpoints)
	return err
}
