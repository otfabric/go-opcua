...
func TestOptions(t *testing.T) {
	...
	notFoundError := func(msg, name string) error {
		switch runtime.GOOS {
		case "windows":
			return fmt.Errorf("opcua: invalid %s: open %s: The system cannot find the file specified.", msg, name)
		default:
			return fmt.Errorf("opcua: invalid %s: open %s: no such file or directory", msg, name)
		}
	}
