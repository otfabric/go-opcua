...
// Seed is a one-time bootstrap helper for interop/capabilities.json and
// interop/coverage.json. It refuses to overwrite existing files.
//
// Source of truth after bootstrap: edit the JSON files directly, then
// regenerate COVERAGE.md with:
//
//	go run ./internal/cmd/render-interop-coverage
//
// Bootstrap (only when JSON files are absent):
//
...
func main() {
	...
	for _, p := range []string{capPath, covPath} {
		if _, err := os.Stat(p); err == nil {
			fmt.Fprintf(os.Stderr, "refusing to overwrite existing %s; edit JSON directly (seed is bootstrap-only)\n", p)
			os.Exit(1)
		}
	}
	caps, entries := build()
	mustWrite(capPath, map[string]any{"capabilities": caps})
	mustWrite(covPath, map[string]any{
		"interopVersion": pin,
		"entries":        entries,
	})
