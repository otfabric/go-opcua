...
func (s *Subscription) pump(ctx context.Context, notifyCh chan<- *DataChangeMessage, cb MsgHandler) {
	for {
		...
		case msg := <-s.internalNotifyCh:
			if msg.Error != nil {
				// Per OPC-UA Part 4 §5.13.5, a PublishResponse can have both a service-level
				// error and notification data. In practice this is rare; we handle the error first.
				s.sendError(msg.Error)
				continue
			}

			if msg.SubscriptionID != s.sub.SubscriptionID {
				s.sendError(fmt.Errorf("%w: message sub id %v does not match sub id %v", errors.ErrInvalidSubscriptionID, msg.SubscriptionID, s.sub.SubscriptionID))
				continue
			}

			// Backpressure detection: check if the notification channel is full.
			// The len() check is inherently racy but acceptable as a heuristic.
			...
			case *ua.DataChangeNotification:
				for _, item := range v.MonitoredItems {
					...
					if !ok {
						out.Error = fmt.Errorf("%w: handle %d", errors.ErrMonitoredItemNotFound, item.ClientHandle)
						// Error is delivered in out.Error; the handler callback receives it
						// via the DataChangeMessage, so no separate propagation is needed.
					} else {
						out.NodeID = nid
			...
			default:
				s.sendError(fmt.Errorf("%w: %T", errors.ErrInvalidResponseType, msg.Value))
			}
		}
	}
}
...
func (s *Subscription) RemoveMonitorItems(ctx context.Context, items ...Item) error {
	...
	for _, item := range items {
		...
		if !ok {
			return fmt.Errorf("%w: item %v", errors.ErrMonitoredItemNotFound, item.id)
		}
	...
	if len(resp.Results) != len(toRemove) {
		return fmt.Errorf("%w: unmonitor items response length mismatch", errors.ErrInvalidResponseType)
	}
...
func (s *Subscription) ModifyMonitorItems(ctx context.Context, nodes ...Request) error {
	...
	if len(resp.Results) != len(toModify) {
		return fmt.Errorf("%w: modify monitored items response length mismatch", errors.ErrInvalidResponseType)
	}
...
func (s *Subscription) SetMonitoringMode(ctx context.Context, monitoringMode ua.MonitoringMode, items ...Item) error {
	...
	if len(resp.Results) != len(toSet) {
		return fmt.Errorf("%w: set monitoring mode response length mismatch", errors.ErrInvalidResponseType)
	}
